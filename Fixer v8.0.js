...
// @version      8.0
// @description  description  Fixes broken Batoto images using an advanced hybrid engine that rewrites k/kXX hosts to n/nXX, performs background server probing, and applies smart prioritization. Optimized for Firefox and Chromium-based browsers with banner-safe handling, proper srcset support, smart number retention, and strong cross-browser stability.
...
(function () {
  ...
  const IS_CHROMIUM = /Chrome\/|Chromium\/|Edg\/|OPR\//.test(navigator.userAgent);

  const PROBE_TIMEOUT = 5000;
  const MAX_ATTEMPTS = 30;
  const RETRY_DELAY = 1000;
  const AGGRESSIVE_INTERVAL = IS_FIREFOX ? 15000 : 2000;
  const ENABLE_EAGER_ON_INITIAL = !IS_FIREFOX;

  const FALLBACK_PREFIXES = ['n', 'x', 't', 's', 'w', 'm', 'c', 'u', 'k'];
  const FALLBACK_ROOTS = ['mbdny.org', 'mbrtz.org', 'bato.to', 'mbwbm.org', 'mbznp.org', 'mbqgu.org'];

  const SUBDOMAIN_RE = /^https?:\/\/([a-z]+)(\d{1,3})\.([a-z0-9\-]+)\.([a-z]{2,4})(\/.*)$/i;
  const HOST_RE = /(^|\/\/)([a-z])(\d+)(\.)/i;
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        if (!timedOut) {
          ...
          else { failedCache.add(cacheKey); reject('empty'); }
        }
      };
      img.onerror = () => {
        if (!timedOut) {
  ...
  function quickFixURL(url) {
    ...
    return url.replace(HOST_RE, function (_match, p1, p2, p3, p4) {
      return `${p1}n${p3}${p4}`;
    });
  }
  ...
  function generateCandidates(parsed) {
    ...
    FALLBACK_PREFIXES.forEach(letter => {
      if (letter !== parsed.prefix) add(letter, parsed.number, parsed.root, parsed.tld, 2);
    });

    [1, 2, 3, 4, 5, 0, 8, 10].forEach(i => {
      if (i !== parsed.number) add(parsed.prefix, i, parsed.root, parsed.tld, 3);
    });

    FALLBACK_ROOTS.forEach(root => {
      const [r, t] = root.split('.');
      if (r !== parsed.root) add(parsed.prefix, parsed.number, r, t, 4);
    });

    return [...new Set(candidates.sort((a, b) => a.priority - b.priority).map(c => c.url))].slice(0, MAX_ATTEMPTS);
  }
  ...
  function rewriteSrcset(srcset, workingUrl) {
    ...
    return srcset.replace(/https?:\/\/[a-z]+\d{1,3}\.[a-z0-9\-]+\.[a-z]{2,4}/gi, newBase);
  }
  ...
  async function advancedFixImage(img, isRetry = false) {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      try {
        await probeUrl(url, PROBE_TIMEOUT + (i > 5 ? 1000 : 0));
        ...
        lastError = e;
        if (e === 'timeout' && i > 8) break;
      }
    }

    if (!isRetry && lastError === 'timeout') {
      setTimeout(() => advancedFixImage(img, true), RETRY_DELAY);
    }
    processingImages.delete(img);
  }
  ...
  function preemptiveQuickFix(img) {
    ...
    const newUrl = img.src.replace(SUBDOMAIN_RE, (m, p1, p2, p3, p4, p5) => `https://n${p2}.${p3}.${p4}${p5}`);

    img.dataset.btfxOrig = img.src;
    img.referrerPolicy = "no-referrer";
    img.src = newUrl;
    if (IS_CHROMIUM && ENABLE_EAGER_ON_INITIAL) img.loading = 'eager';
    img.dataset.btfxStatus = "preemptive";

    setTimeout(() => {
        if (img.complete && img.naturalWidth === 0) advancedFixImage(img);
    }, 2000);
  }
  ...
  function rewrite(img) {
    ...
    for (const attr of attributes) {
      ...
      if (attr === 'srcset') {
        const newValue = value.split(',').map(p => {
        ...
        if (newValue !== value) {
          img.setAttribute(attr, newValue);
          touched = true;
        }
      } else {
  ...
  const observer = new MutationObserver(mutations => {
    mutations.forEach(m => {
      m.addedNodes.forEach(node => node.nodeType === 1 && scan(node));
  ...
  observer.observe(document.documentElement, {
    childList: true, subtree: true, attributes: true, attributeFilter: ['src', 'srcset', 'data-src']
  });
  ...
  (function updater() {
    ...
    const check = (m) => {
      GM_xmlhttpRequest({
        method: 'GET', url: UPDATE_URL, onload: (res) => {
          const ver = (res.responseText || '').match(/@version\s+([0-9.]+)/i);
          if (ver && isRemoteNewer(ver[1], VERSION)) {
            if (confirm(`[BTFX] Update available: v${VERSION} -> v${ver[1]}`)) window.open(UPDATE_URL, '_blank');
          } else if (m) alert(`BTFX v${VERSION} is up to date. [${IS_FIREFOX ? 'Firefox' : 'Chromium'}-optimized]`);
        }
      });
    };
    setTimeout(check, 5000);
    if (typeof GM_registerMenuCommand === 'function') GM_registerMenuCommand('BTFX: Check Updates', () => check(true));
  })();
